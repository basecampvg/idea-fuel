...
export default function DailyPickDetailScreen() {
  const { dateLocal } = useLocalSearchParams<{ dateLocal: string }>();
  ...
  const formattedDate = dateLocal
    ? new Date(dateLocal + 'T12:00:00').toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
    ...
    return (
      <SafeAreaView style={styles.container} edges={['bottom']}>
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyTitle}>No Pick</Text>
          <Text style={styles.emptyText}>
            No daily trend pick was recorded for {formattedDate}.
          </Text>
        </View>
      </SafeAreaView>
    );
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        {winnerReport?.purchase_proof && (
          <View style={styles.sectionCard}>
            ...
            <View style={styles.sectionContent}>
              <Text style={styles.sectionLabel}>Buy Stage</Text>
              <Text style={styles.sectionValue}>
                {winnerReport.purchase_proof.buy_stage}
              </Text>
              <Text style={styles.sectionLabel}>Evidence</Text>
              {winnerReport.purchase_proof.evidence.map((e, i) => (
        ...
        {/* Why This Won */}
        {winnerReport?.transparency && (
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}>
              <Ionicons name="checkmark-circle" size={20} color={colors.green} />
              <Text style={styles.sectionTitle}>Why This Pick Won</Text>
            </View>
            <View style={styles.sectionContent}>
              <Text style={styles.sectionLabel}>Reasons</Text>
              {winnerReport.transparency.winner_reason.map((reason, i) => (
                <Text key={i} style={styles.listItem}>
                  • {reason}
                </Text>
              ))}
              <Text style={[styles.sectionLabel, { marginTop: 12 }]}>Evidence</Text>
              {winnerReport.transparency.evidence.map((e, i) => (
                <Text key={i} style={styles.listItem}>
                  • {e}
                </Text>
              ))}
